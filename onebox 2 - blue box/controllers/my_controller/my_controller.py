...
class RobotController(Supervisor):  # Use Supervisor instead of Robot
    def __init__(self):
        ...
        self.sector_coordinates = {
            ...
            "Blue": [-0.5725307377437417, -3.57287519101565],  
            "Green": [-0.5725307377437417, -1.57287519101565], #-0.6875144492298266, -2.5710764570773796 
    ...
    def detect_box_camera(self,color):
        """
    ...
    def detect_and_pick_box(self,color):
        # Detect the box
        global detected_color
        result = self.detect_box_camera(color)
        if result:
    ...
    def loop(self):
        while self.step(self.timestep) != -1:
            ...
            self.StandStill()
            self.detect_and_pick_box(color_matrixx[0])
            self.StandStill()
            self.navigate_to_sector("Center")
            self.StandStill()
            self.navigate_to_sector("Wall")
